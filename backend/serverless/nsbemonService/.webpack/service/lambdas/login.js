...
var handler = exports.handler = function handler(event, context, callback) {
  ...
  _jsonwebtoken2.default.sign({ user: user }, process.env.JWT_SECRET, { expiresIn: JWT_EXPIRATION_TIME }, function (error, token) {
    ...
    if (error) {
      response = {
        ...
        body: (0, _stringify2.default)({ error: error })
      };
    } else {
      response = {
        statusCode: 200,
